<template>
  <div class="inbound-plans-container">
    <div class="header">
      <h1>入库计划</h1>
    </div>
    ...
    <WarehousingOrderForm
      v-model:visible="warehousingOrderFormVisible"
      :isEdit="false"
      :warehousingOrderData="warehousingOrderPrefill"
      @success="handleWarehousingOrderSuccess"
    />
  </div>
</template>
...
import type { InboundPlan, InboundPlanQueryParams } from '@/types'
  ...
  plans.value.forEach((plan, planIndex) => {
    ...
    if (items.length === 0) {
      ...
      items.forEach((item: any, index: number) => {
        result.push({
          ...plan,
          row_key: `${plan.inbound_plan_id}_${index}`,
...
const searchParams = reactive<InboundPlanQueryParams>({
  page: 1,
  pageSize: 100,
  planNumber: '',
  contractNumber: '',
...
const handleGenerateWarehousingOrder = (record: any) => {
  ...
  // 映射计划明细到入库明细
  const warehousingItems = items.map((item: any, index: number) => ({
    no: index + 1,
    product_code: item.product_code || '',
    product_name: item.product_name || '',
    model: item.model || '',
  ...
  warehousingOrderPrefill.value = {
    ...
    customer_name: plan.supplier_name || '',
...
const handleDelete = (record: any) => {
  ...
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除入库计划 ${plan.plan_number} 吗？`,
    onOk: async () => {
      try {
        await inboundPlansApi.delete(plan.inbound_plan_id)
...
const handleExport = async () => {
  try {
    ...
    const response = await inboundPlansApi.getAll(params)
    const allPlans: InboundPlan[] = response.data || []
    const rows: any[] = []
    ...
    exportToExcel({ filename: '入库计划', columns: filteredExportColumns, data: rows })
  } catch {
    message.error('导出失败')
  }
}
