...
const columns = [
  {
    ...
    onFilter: (value: string, record: Receivable) =>
  ...
  },
  {
    ...
    onFilter: (value: number, record: Receivable) => record.source_bill_type === value,
  },
  {
    title: '来源单据编号',
  ...
  },
  {
    ...
    onFilter: (value: number, record: Receivable) => record.status === value,
  },
  {
    title: '开票状态',
    ...
    onFilter: (value: number, record: Receivable) => record.billing_status === value,
  },
  {
    title: '结算日期',
    dataIndex: 'due_date',
...
const onSelectChange = (keys: string[], rows: Receivable[]) => {
...
const handleGenerateStatement = () => {
  ...
  // 检查是否都是同一客户
  const customerIds = [...new Set(selectedRecords.value.map(r => r.customer_id))]
  if (customerIds.length > 1) {
    message.warning('请选择同一客户的记录生成对账单')
    return
  }

  // 检查是否包含出库单类型的记录
  const hasDeliveryOrders = selectedRecords.value.some(r => r.source_bill_type === 1)
  if (!hasDeliveryOrders) {
    message.warning('请选择包含出库单的记录')
    return
  }
...
const handleGenerateWriteOff = () => {
  ...
  const customerIds = [...new Set(selectedRecords.value.map(r => r.customer_id))]
  if (customerIds.length > 1) {
    message.warning('请选择同一客户的记录生成核销单')
    return
  }
...
const handleDeleteRecord = async (record: Receivable) => {
  try {
    await receivablesApi.delete(record.receivable_id)
