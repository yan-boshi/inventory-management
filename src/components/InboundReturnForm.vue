    ...
    <div v-if="visible" class="return-order-form">
      ...
      <div class="return-order-header">
        <div class="form-row">
          ...
          <div class="form-item">
            <label class="form-label">原入库单号：</label>
        ...
        <div class="form-row">
          ...
          <div class="form-item">
            ...
            <a-input v-model:value="form.supplier_name" class="invisible-input" disabled />
          </div>
        </div>
      </div>
...
import { createInboundReturn, updateInboundReturn, getNewInboundReturnOrderNumber } from '@/api/inboundReturns'
...
const itemColumns = [
  ...
  { title: '原入库数量', key: 'original_quantity', width: '8%', align: 'right' },
...
const getNewOrderNumber = async () => {
  try {
    const response = await getNewInboundReturnOrderNumber()
...
const loadSourceOrderItems = () => {
  ...
  form.source_order_id = sourceOrder.warehousing_order_id || sourceOrder.id || ''
  form.source_order_number = sourceOrder.order_number || ''
  form.contract_number = sourceOrder.contract_number || ''
  form.supplier_name = sourceOrder.supplier_name || sourceOrder.customer_name || ''
  form.supplier_code = sourceOrder.supplier_code || ''
  form.currency = sourceOrder.currency || 'CNY'
  form.return_person = getCurrentUsername()

  const sourceItems = JSON.parse(sourceOrder.warehousing_items || '[]')
...
watch(() => props.visible, async (newVal) => {
  if (newVal) {
    if (props.editData) {
      ...
      form.supplier_code = data.supplier_code || ''
...
const handleSubmit = async () => {
  ...
  for (const item of form.return_items) {
    ...
    if (item.quantity > item.original_quantity) {
      message.error(`商品 ${item.product_name} 的退货数量不能超过原入库数量`)
      return
    }
  }
  ...
  try {
    ...
    if (isEdit.value && props.editData) {
      await updateInboundReturn(props.editData.inbound_return_id, submitData)
      message.success('入库退货单更新成功')
    } else {
      await createInboundReturn(submitData)
      message.success('入库退货单创建成功')
    }
