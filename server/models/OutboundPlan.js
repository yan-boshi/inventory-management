...
class OutboundPlan extends BaseModel {
  constructor() {
    super('outbound_plans', 'outbound_plan_id')
  }
  ...
  async generatePlanNumber() {
    ...
    const query = `
      ...
      LIMIT 1
    `
    const [result] = await pool.query(query, [`XSD-CK-${dateStr}-%`])
    ...
    return `XSD-CK-${dateStr}-${sequence.toString().padStart(3, '0')}`
  }

  async create(data) {
    const planData = {
      outbound_plan_id: data.outbound_plan_id || generateUUID(),
...
export default new OutboundPlan()
