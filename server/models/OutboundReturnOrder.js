...
class OutboundReturnOrder extends BaseModel {
  constructor() {
    super('outbound_return_orders', 'outbound_return_id')
  }
  ...
  async generateOrderNumber() {
    ...
    const query = `
      ...
      LIMIT 1
    `
    const [result] = await pool.query(query, [`XSD-OR-${dateStr}-%`])
    ...
    return `XSD-OR-${dateStr}-${sequence.toString().padStart(3, '0')}`
  }
  ...
  async create(data) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const orderData = {
          outbound_return_id: generateUUID(),
...
export default new OutboundReturnOrder()
