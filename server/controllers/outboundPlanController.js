import OutboundPlan from '../models/OutboundPlan.js'
import pool from '../config/database.js'

export const getAllOutboundPlans = async (req, res) => {
  try {
    const {
      page = 1,
      pageSize = 10,
    ...
    const result = await OutboundPlan.paginate({
...
export const getOutboundPlanById = async (req, res) => {
  try {
    const { id } = req.params
    const plan = await OutboundPlan.findById(id)
    if (!plan) {
      return res.status(404).json({ success: false, message: 'Outbound plan not found' })
    }
...
export const createOutboundPlan = async (req, res) => {
  try {
    const {
      sales_order_id,
      contract_number,
    ...
    const plan = await OutboundPlan.create({
      sales_order_id,
      contract_number,
      customer_name,
      plan_items,
...
export const updateOutboundPlan = async (req, res) => {
  try {
    const { id } = req.params
    const existing = await OutboundPlan.findById(id)
    if (!existing) {
      return res.status(404).json({ success: false, message: 'Outbound plan not found' })
    }
    ...
    const plan = await OutboundPlan.update(id, updateData)
    res.json({ success: true, data: plan })
  } catch (error) {
    res.status(500).json({ success: false, message: error.message })
  }
...
export const deleteOutboundPlan = async (req, res) => {
  try {
    const { id } = req.params
    const existing = await OutboundPlan.findById(id)
    if (!existing) {
    ...
    await OutboundPlan.delete(id)
    res.json({ success: true, message: 'Outbound plan deleted' })
  } catch (error) {
    res.status(500).json({ success: false, message: error.message })
  }
}
...
export const getNewPlanNumber = async (req, res) => {
  try {
    const planNumber = await OutboundPlan.generatePlanNumber()
...
export const generateFromSalesOrder = async (salesOrder) => {
  try {
    // 先删除该销售订单关联的旧出库计划
    await OutboundPlan.deleteBySalesOrderId(salesOrder.sales_order_id)
    ...
    console.error('Generate outbound plan from sales order failed:', error)
    throw error
  }
}
